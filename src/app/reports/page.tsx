...
import { BarChart3, Users, CalendarDays, Download } from 'lucide-react';
import {
...
const STATUS_BADGE: Record<AttendanceStatus, string> = {
  ...
  'Leave Informed': 'bg-yellow-100 text-yellow-800',
};
...
export default function ReportsPage() {
  ...
  const filteredMeetings = meetings.filter((m) => {
    if (dateFrom && dateTo) {
      try {
        return isWithinInterval(parseISO(m.MeetingDate), {
  ...
  const meetingSummary = filteredMeetings.map((m) => {
    ...
    return {
      ...
      leave: att.filter((a) => a.Status === 'Leave Informed').length,
      total: att.length,
    };
  });
    ...
    <div className="space-y-5">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <div className="flex flex-col gap-1">
              ...
              <Select value={filterMember} onValueChange={setFilterMember}>
                ...
                <SelectContent>
                  ...
                  {members.map((m) => (
                    <SelectItem key={m.MemberID} value={m.MemberID}>{m.FullName}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            <div className="flex items-end">
              <Button
                ...
                size="sm"
                className="h-8"
                onClick={() => { setDateFrom(''); setDateTo(''); setFilterMeeting(null); setFilterMember(null); setFilterArea(null); }}
              >
                Clear
              </Button>
            </div>
      ...
      <div className="flex gap-2">
        {[
          ...
          { key: 'meeting', label: 'Meeting-wise' },
                  ...
                  {filteredMembers.map((m, i) => {
                    ...
                    return (
                      <tr key={m.MemberID} className={i % 2 === 0 ? 'bg-background' : 'bg-muted/30'}>
                        <td className={`p-2 px-3 border font-medium sticky left-0 z-10 ${i % 2 === 0 ? 'bg-background' : 'bg-muted/30'}`}>
                          {m.FullName}
                        </td>
                        <td className={`p-2 px-3 border text-muted-foreground sticky left-[140px] z-10 ${i % 2 === 0 ? 'bg-background' : 'bg-muted/30'}`}>
                          {m.Area}
                        </td>
                        {filteredMeetings.map((mt) => {
                          ...
                          return (
                            <td key={mt.MeetingID} className="border text-center">
                              {status ? (
                                <span className={`inline-block w-6 h-6 rounded leading-6 text-xs font-bold ${STATUS_CELL[status]}`}>
                                  {STATUS_ABBR[status]}
                                </span>
                              ) : (
      ...
      {tab === 'meeting' && (
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-base">Meeting-wise Summary</CardTitle>
          </CardHeader>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-green-700">Present</TableHead>
                  <TableHead className="text-red-700">Absent</TableHead>
                  <TableHead className="text-yellow-700">Leave</TableHead>
              ...
              <TableBody>
                {loading ? (
                  ...
                  <TableRow><TableCell colSpan={8} className="text-muted-foreground">No data</TableCell></TableRow>
                ) : (
      ...
      {tab === 'member' && (
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-base">Member-wise Attendance History</CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {loading ? (
                  ...
                  <TableRow><TableCell colSpan={7} className="text-muted-foreground">No data</TableCell></TableRow>
                ) : (
                  memberSummary.map((m) => (
